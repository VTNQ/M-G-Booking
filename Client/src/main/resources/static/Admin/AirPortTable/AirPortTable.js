...
$('#prevPage').click(function () {
    if (CurrentPage > 1) {
        CurrentPage--;
        fetchAirPort(CurrentPage, pageSize);
    }
})
...
function changePage(page) {
    if (page >= 1 && page <= totalPages) {
        CurrentPage = page;
        fetchAirPort(CurrentPage - 1, pageSize); // Điều chỉnh chỉ mục trang (bắt đầu từ 0)
    }
}

function fetchAirPort(page, size) {
    ...
    const url = `http://localhost:8686/AirPort/${id}?page=${page}&size=${size}`;
    $.ajax({
        url: url,
        method: 'GET',
        headers: {
