...
function changePage(page){
    if (page >= 1 && page <= totalPages) {
        CurrentPage = page;
        fetchCity(CurrentPage, pageSize);  // Fetch countries for the selected page
    }
}
function fetchCity(page,size){
    ...
    const url = `http://localhost:8686/City/FindCityPage/${id}?page=${page}&size=${size}`;
    $.ajax({
        url: url,
        method: 'GET',
        headers: {
...
$('#prevPage').click(function (){
    if(CurrentPage>1){
        CurrentPage--;
        fetchCity(CurrentPage,pageSize);
    }
})
